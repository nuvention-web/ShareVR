...
namespace IBM.Watson.DeveloperCloud.Utilities
{
	...
	public class Config
	{
		...
		{
			try {
				// ShareVR - Modified to be able to run in Unity Editor mode
				LoadConfig (GlobalParameters.WatsonConfigJSON);

			} catch (System.IO.FileNotFoundException) {
				// mark as loaded anyway, so we don't keep retrying..
				UnityEngine.Debug.LogError ("ShareVR (Watson): " + "Failed to load config file.");
				ConfigLoaded = true;
			}
		}
